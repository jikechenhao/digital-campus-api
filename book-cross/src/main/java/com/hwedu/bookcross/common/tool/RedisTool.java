...
public class RedisTool {
    ...
    static {
        if (pool == null) {
            pool = new JedisPool(new JedisPoolConfig(), Variables.getInstance().getRedisIp(), Variables.getInstance()
                    .getRedisPort());// 使用连接池
            // jedis = new Jedis(Variables.REDIS_IP, Variables.REDIS_PORT);// 直接连接
    ...
    /**
     * @param lKey  Set集合的key
     * @param start 集合的起始位置
     * @param end   集合的末位，如何end=-1取全部数据
     * @return 根据key值返回一个集合
     */
